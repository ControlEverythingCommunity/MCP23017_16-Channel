...
{
  // Set variable
  Particle.variable("i2cdevice", "MCP23017_I2CR16G5LE");
...
void loop()
{
  ...
  // Stop I2C transmission
  Wire.endTransmission();
  Particle.publish("Pull-up enabled on all GPIO pins of", + "Port-A");
  delay(1000);
  ...
  for (int i = 0; i < 8; i++)
  {
    ...
    if (state == value)
    {
      Particle.publish("Port-A GPIO Pin ", String(i) + " is High");
    }
    else
    {
      Particle.publish("Port-A GPIO Pin ", String(i) + " is Low");
    }
    value = value << 1;
    delay(1000);
  }
  ...
  // Stop I2C transmission
  Wire.endTransmission();
  Particle.publish("Pull-up enabled on all GPIO pins of", + "Port-B");
  delay(1000);
  ...
  for (int i = 0; i < 8; i++)
  {
    ...
    if (state == value)
    {
      Particle.publish("Port-B GPIO Pin ", String(i) + " is High");
    }
    else
    {
      Particle.publish("Port-B GPIO Pin ", String(i) + " is Low");
    }
    value = value << 1;
    delay(1000);
  }
